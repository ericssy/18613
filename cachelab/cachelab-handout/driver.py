...
def main():
    ...
    # Check for handin.tar and untar


    retcode = subprocess.call(['tar','-xvf','handin.tar','csim.c','trans.c'])

    if(retcode == 0):
        print "done"
    else:
        print "Either handin.tar is missing. Run \"make\" again. OR You do not have all the files required"
        return
    ...
    if autograde:
        autoresult="{\"scores\": {\"Autograded\": %.1f}, \"scoreboard\": [%.1f, %d, %d, %d, %d]}" % (total_score, total_score, cycles32+cycles63, cycles32, cycles63)
        print "%s" % autoresult
