...
static block_t *LIFO_free(block_t *block);
static block_t *find_fit_original(size_t asize);
static void allocate_and_split(block_t *block, size_t asize);
static void delete_from_list(block_t * block);
...
{   
    ...
    // if there's only one freed block, it means root will be allocated
    // set root to null and extend the heap
    // else set the root to root->next
    // if (free_block_root == find_next_free(free_block_root))
    // {
    //     free_block_root = NULL;
    ...
    // {
    //     dbg_printf("\nFIND TEST\n");
    //     free_block_root = find_next_free(free_block_root);
    // }

    // write_next(find_prev_free(block), find_next_free(block));
    // write_prev(find_next_free(block), find_prev_free(block));
    // if (free_block_root == block && find_next_free(free_block_root) == free_block_root) 
    // {
    //     free_block_root = NULL;
    //     extend_heap(chunksize);
    // } else if (free_block_root == block)
    // {
    //     free_block_root = find_next_free(free_block_root);
    // }
    ...
    allocate_and_split(block, asize);
...
static void allocate_and_split(block_t *block, size_t asize) {
    ...
    if ((block_size - asize) >= min_block_size + dsize)
    {
        ...
        {
            // when root is the block being allocated and there's other nodes in the list
            free_block_root = NULL;
            extend_heap(chunksize);
            return;
...
static block_t *find_fit(size_t asize)
{
    ...
    if (get_size(free_block_root) >= asize)
    {
        return free_block_root;
    }

    for (block = find_next_free(free_block_root); get_size(block) > 0; block = find_next_free(block))
    {
        if (block == free_block_root) {
            return NULL;
        }
...
bool mm_checkheap(int line)
{
    ...
    {
        printf("Line: %d, epilogue and prologue error \n", line);
        return false;
    }
...
static block_t* LIFO_free(block_t *block) {
    ...
    if (prev_alloc && next_alloc) // Case 1     both are allocated
    {
        ...
        block_t *root_next = find_next_free(free_block_root);
        write_next(free_block_root, block);
    ...
    else if (!prev_alloc && next_alloc) // Case 3   prev free and next allocated
    {
        ...
        if (free_block_root == NULL)
        {
            ...
            block_t *root_next = find_next_free(free_block_root);
            write_next(free_block_root, block_prev);
            write_prev(root_next, block_prev);
            write_next(block_prev, root_next);
            write_prev(block_prev, free_block_root);
...
static void delete_from_list(block_t * block)
{
    if (block == free_block_root && find_next_free(block) == block)
    {
        free_block_root = NULL;
        return;
    }
