...
static block_t *extend_heap(size_t size)
{
    ...
    if (free_block_root == NULL) {
        ...
        dbg_printf("root address: %p", free_block_root);
    } else {
        // if there's already free block
        block_t *tail = find_prev_free(free_block_root); // the prev of root is the tail
        dbg_printf("root address: %p", free_block_root);
        dbg_printf("tail address: %p", free_block_root->payload.free_payload.prev);
        // tail -> next = block
        // block -> prev = tail
    ...
    dbg_printf("prev address: %p", block->payload.free_payload.prev);
    dbg_printf("next address: %p", block->payload.free_payload.next);
...
static block_t *coalesce_block(block_t *block)
{
    ...
    {
        dbg_printf("prev free next alloc");
        dbg_printf("B prev : %p ", block_prev);
