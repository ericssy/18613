...
static void write_header_eliminating_footer(block_t *block, size_t size, bool alloc, bool prev_alloc);
static bool get_prev_alloc(block_t * block);
static word_t pack_eliminating_footer(size_t size, bool alloc, bool prev_alloc);
...
bool mm_init(void)
{
    ...
    start[1] = pack_eliminating_footer(0, true, true);
...
void free(void *bp) // bp points to the payload
{
    ...
    dbg_printf("DEBUG: previous alloc %lx", block->header);
    write_header_eliminating_footer(block, size, false, get_prev_alloc(block));
...
static block_t *extend_heap(size_t size)
{
    ...
    write_header_eliminating_footer(block, size, false, prev_alloc_on_heap);
    write_footer(block, size, false);
    ...
    if (check_if_seg_list_empty()) {
        ...
        write_header_eliminating_footer(block_next, 0, true, false);
        //dbg_ensures(mm_checkheap(__LINE__));
    } else {
...
static void allocate_and_split(block_t *block, size_t asize, int index) {
    ...
    if ((block_size - asize) >= min_block_size + dsize)
    {
        ...
        write_header_eliminating_footer(block, asize, true, true);
        //write_footer(block, asize, true);
        ...
        //write_header(block_next, block_size - asize, false);
        write_header_eliminating_footer(block_next, block_size - asize, false, true);
        write_footer(block_next, block_size - asize, false);
        ...
        delete_from_list(block, global_index);
        block_t * block_next = find_next(block);
        write_header_eliminating_footer(block, block_size, true, true);

        write_header_eliminating_footer(block_next, get_size(block_next), get_alloc(block_next), true);
...
bool mm_check_seg_list_consistency(int line)
{
    ...
    if (free_block_in_heap != count_next)
    {
...
static block_t* LIFO_free(block_t *block, int index) {
    ...
    if (prev_alloc && next_alloc) // Case 1     both are allocated
    {
        ...
        write_header_eliminating_footer(block, size, false, true);
        write_footer(block, size, false);
        // change the header of next block, current alloc, prev free
        write_header_eliminating_footer(block_next, get_size(block_next), true, false);
        dbg_requires(!get_alloc(block));
        dbg_printf("\nTEST\n");
    ...
    else if (prev_alloc && !next_alloc) // Case 2   prev allocated and next free
    {
        ...
        write_header_eliminating_footer(
                find_next(block), get_size(find_next(block)), true, false);
        dbg_requires(!get_alloc(block));
    ...
    else if (!prev_alloc && next_alloc) // Case 3   prev free and next allocated
    {
        ...
        //write_header(block_prev, size, false);
        write_header_eliminating_footer(block_prev, size, false, true);
        write_footer(block_prev, size, false);
        write_header_eliminating_footer(block_next, get_size(block_next), true, false);
        block = block_prev;
    ...
    else // Case 4      both are free
    {
        ...
        write_footer(block_prev, size, false);
        write_header_eliminating_footer(
                find_next(block_prev), get_size(find_next(block_prev)), true, false);
...
static void write_header_eliminating_footer(block_t *block, size_t size, bool alloc, bool prev_alloc)
{
    ...
    {
        block->header = size | (alloc_mask & alloc);
    }
    dbg_printf(" WRITE Header opt:  %p, alloc %d, prev_alloc %d. ", block, alloc, prev_alloc);
...
static word_t pack_eliminating_footer(size_t size, bool alloc, bool prev_alloc)
{
    ...
    {
        return size | (alloc_mask & alloc);
    }
}

// static void pack_prev_alloc_only(block_t *block, bool prev_alloc)
// {

//     block->header = block->header & (
// }

// static word_t *header_to_end_of_block(block_t *block)
// {
//     return (word_t *)(block->payload.alloc_payload + get_size(block) - dsize);
// }

// static void modify_alloc_only(block_t block)
// {
