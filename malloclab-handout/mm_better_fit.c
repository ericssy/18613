...
// functions for better fit algorithm
...
static block_t* LIFO_free(block_t *block, int index) {
    ...
    if (prev_alloc && next_alloc) // Case 1     both are allocated
    {
        ...
        else if (size <= get_size(segregated_list[global_index]))
        {
            block_t *root_prev = find_prev_free(segregated_list[global_index]);
            write_prev(segregated_list[global_index], block);
            write_next(root_prev, block);
            write_next(block, segregated_list[global_index]);
            write_prev(block, root_prev);
        }
        else 
        {
            block_t *target = find_better_fit_target(segregated_list[global_index], size);
            block_t *target_next = find_next_free(target);
            write_next(target, block);
            write_prev(target_next, block);
            write_next(block, target_next);
            write_prev(block, target);
        }
    ...
    else if (!prev_alloc && next_alloc) // Case 3   prev free and next allocated
    {
        ...
        {
            block_t *root_prev = find_prev_free(segregated_list[global_index]);
            write_prev(segregated_list[global_index], block_prev);
            write_next(root_prev, block_prev);
            write_next(block_prev, segregated_list[global_index]);
            write_prev(block_prev, root_prev);
        }
        else 
        {
            block_t *target = find_better_fit_target(segregated_list[global_index], size);
            block_t *target_next = find_next_free(target);
            write_next(target, block_prev);
            write_prev(target_next, block_prev);
            write_next(block_prev, target_next);
            write_prev(block_prev, target);
        }
